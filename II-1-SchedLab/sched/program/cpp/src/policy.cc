...
void update(int time)
{
  for(list<E>::iterator i=Q_cpu.begin();i!=Q_cpu.end();i++)
  {
    ...
    //i->score=1.0*(time-i->arrivaltime)/(i->deadline-i->arrivaltime+USED_SCORE*i->used_times);
    //i->score=i->priority_score*(i->score)*t;
...
Action policy(const std::vector<Event>& events, int current_cpu,
  ...
  //cout<<current_cpuid.id<<' '<<current_cpuid.used_times<<' ';
  //cout<<current_ioid.id<<' '<<current_ioid.used_times<<' ';
  ...
  for(unsigned int i=0;i<events.size();i++)
  {
    ...
    if(events[i].type==Event::Type::kTaskArrival)
    {
      if(queue_bool.insert(events[i].task.taskId).second)
      {
        ...
        tmp.used_times=0;
        tmp.arrivaltime=events[i].task.arrivalTime;
